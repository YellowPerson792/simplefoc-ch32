...
void Encoder::attachInterruptHelper(uint32_t pin, void (*callback)(void), int mode) {
...
void Encoder::enableInterrupts(void (*doA)(), void(*doB)(), void(*doIndex)()){
  ...
  switch(quadrature){
    ...
    case Quadrature::OFF:
      // A callback and B callback
      if(doA != nullptr) attachInterruptHelper(digitalPinToInterrupt(pinA), doA, RISING);
      if(doB != nullptr) attachInterruptHelper(digitalPinToInterrupt(pinB), doB, RISING);
      break;
  }

  // if index used initialize the index interrupt
  if(hasIndex() && doIndex != nullptr) attachInterruptHelper(digitalPinToInterrupt(index_pin), doIndex, CHANGE);
}
