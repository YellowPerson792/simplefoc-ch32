...
void _setPwm(int ulPin, uint32_t value, int resolution)
{
  ...
  uint32_t channel = CH_PIN_CHANNEL(pinmap_function(pin, PinMap_PWM));
  HT->setCaptureCompare(channel, value, (TimerCompareFormat_t)resolution);
}

void TIM_TimeBaseInitHelper(TIM_HandleTypeDef *htim){
  TIM_TypeDef *Instance = htim->Instance;
  TIM_TimeBaseInitTypeDef InitStructure = htim->Init;
  TIM_TimeBaseInit(Instance, &InitStructure);
}
...
HardwareTimer* _initPinPWM(uint32_t PWM_freq, int ulPin)
{
  ...
  if (HardwareTimer_Handle[index] == NULL) {
    ...
    HardwareTimer_Handle[index]->handle.Init.TIM_CounterMode = TIM_CounterMode_CenterAligned3;
    TIM_TimeBaseInitHelper(&(HardwareTimer_Handle[index]->handle));
  }
  HardwareTimer *HT = (HardwareTimer *)(HardwareTimer_Handle[index]->__this);
  uint32_t channel = CH_PIN_CHANNEL(pinmap_function(pin, PinMap_PWM));
...
HardwareTimer* _initPinPWMLow(uint32_t PWM_freq, int ulPin)
{
  ...
  if (HardwareTimer_Handle[index] == NULL) {
    ...
    HardwareTimer_Handle[index]->handle.Init.TIM_CounterMode = TIM_CounterMode_CenterAligned3;
    TIM_TimeBaseInitHelper(&(HardwareTimer_Handle[index]->handle));
    TIM_OCInitTypeDef sConfigOC = TIM_OCInitTypeDef();
    sConfigOC.TIM_OCMode = TIM_OCMode_PWM2;
    sConfigOC.TIM_Pulse = 100;
    sConfigOC.TIM_OCPolarity = TIM_OCPolarity_Low;
    sConfigOC.TIM_OCNPolarity = TIM_OCNPolarity_High;
    // sConfigOC.OCFastMode = TIMER_DISABLED;
    sConfigOC.TIM_OCIdleState = TIM_OCIdleState_Set;
    sConfigOC.TIM_OCNIdleState = TIM_OCNIdleState_Reset;
    uint32_t channel = CH_PIN_CHANNEL(pinmap_function(pin, PinMap_PWM));
    TIM_OC4Init(HardwareTimer_Handle[index]->handle.Instance, &sConfigOC);        //to be modified
  }
  HardwareTimer *HT = (HardwareTimer *)(HardwareTimer_Handle[index]->__this);
  uint32_t channel = CH_PIN_CHANNEL(pinmap_function(pin, PinMap_PWM));
...
//     HardwareTimer_Handle[index]->handle.Init.TIM_CounterMode = TIM_COUNTERMODE_CENTERALIGNED3;
//     TIM_TimeBaseInit(&(HardwareTimer_Handle[index]->handle));
//     ((HardwareTimer *)HardwareTimer_Handle[index]->__this)->setOverflow(PWM_freq, HERTZ_FORMAT);  
//   }
//   HardwareTimer *HT = (HardwareTimer *)(HardwareTimer_Handle[index]->__this);
     
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(uhPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, uhPinName);
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(ulPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, ulPinName);
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(vhPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, vhPinName);
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(vlPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, vlPinName);
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(whPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, whPinName);
//   HT->setMode(CH_PIN_CHANNEL(pinmap_function(wlPinName, PinMap_PWM)), TIMER_OUTPUT_COMPARE_PWM1, wlPinName);
...
//   LL_TIM_CC_EnableChannel(HT->getHandle()->Instance, TIM_CHANNEL_CH1 | TIM_CHANNEL_CH1N | TIM_CHANNEL_CH2 | TIM_CHANNEL_CH2N | TIM_CHANNEL_CH3 | TIM_CHANNEL_CH3N);
